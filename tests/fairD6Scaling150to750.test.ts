/**
 * Fair d6 scaling 150→750 at 150 steps, 8 matches per adjacent pair, 20 rounds, fair harness.
...
describe('Fair d6 scaling 150→750 @150 steps, 8 per pair', () => {
  it('adjacent pairs 150vs300, 300vs450, 450vs600, 600vs750 — 8 matches each, fair', async () => {
    ...
    console.log('\n  ═══════════════════════════════════════════════════════════════════════');
    console.log('  FAIR d6 SCALING — 150→750 @150 steps, 8 matches per adjacent pair, 20 rounds');
    console.log('  ═══════════════════════════════════════════════════════════════════════');
    ...
    for (const [low, high] of pairs) {
      ...
      for (let g = 0; g < 8; g++) {
        const gg = groups[g % 4];
        // Test HIGH Chall vs LOW Ref (strong Chall vs weak Ref)
    ...
    console.log('\n  ───────────────────────────────────────────────────────────────────────');
    console.log('  SCALING SUMMARY (HIGH Chall vs LOW Ref, fair, 8 each, depth 6)');
    ...
    console.log(`\n  Avg strength per step (150→750, 4 steps, d6): ${avgStrength>=0?'+':''}${avgStrength.toFixed(3)}/game`);
    console.log('  Expected positive at d6 as higher iterations unlock deeper horizon moves');
    console.log('  ═══════════════════════════════════════════════════════════════════════\n');
  ...
  it('mirrors at each tier 150,300,450,600,750 — 8 each, should be ~0', async () => {
    ...
    console.log('\n  ───────────────────────────────────────────────────────────────────────');
    console.log('  MIRRORS d6 — each tier vs itself, 8 games, fair');
    for (const lvl of levels) {
      ...
      const cW = games.filter(g=>g.winner==='C').length;
      const rW = games.filter(g=>g.winner==='R').length;
      console.log(`  d6@${String(lvl).padStart(3)} vs d6@${String(lvl).padStart(3)}  ${totalC}-${totalR} Δ=${delta>=0?'+':''}${delta} avg=${avg>=0?'+':''}${avg.toFixed(3)} W/L/D ${cW}/${rW}/${8-cW-rW}`);
