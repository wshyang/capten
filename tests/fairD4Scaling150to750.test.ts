/**
 * Fair d4 scaling 150→750 at 150 steps, 8 matches per adjacent pair, 20 rounds, fair harness.
...
describe('Fair d4 scaling 150→750 @150 steps, 8 per pair', () => {
  it('adjacent pairs 150vs300, 300vs450, 450vs600, 600vs750 — 8 matches each, fair', async () => {
    ...
    // also add mirrors for each tier (vs itself) for bias check
    const mirrors = levels.map(l => [l, l] as [number, number]);

    // 8 matches per pair = 2 per 4 groups
    const groups = [
    ...
    console.log('\n  ═══════════════════════════════════════════════════════════════════════');
    console.log('  FAIR d4 SCALING — 150→750 @150 steps, 8 matches per adjacent pair, 20 rounds');
    console.log('  ═══════════════════════════════════════════════════════════════════════');
    ...
    for (const [low, high] of pairs) {
      ...
      // Use low as Challenger (weaker) vs high as Reference (stronger) — expect negative delta if stronger helps
      // For scaling we want high Chall vs low Ref, but also test low Chall vs high Ref to get strength. We'll do both? For now high Chall vs low Ref (strong Chall)
      // Actually to measure scaling, we want stronger Chall vs weaker Ref — so high Chall vs low Ref
      const seeds = seedsFor(8, low); // deterministic per pair
      const games: ReturnType<typeof runFairGame>[] = [];
      for (let g = 0; g < 8; g++) {
        const grp = groups[g % 4]; // 2 per group over 8
        // repeat assignment: g=0->G0,1->G1,2->G2,3->G3,4->G0,5->G1,6->G2,7->G3
        const gg = groups[g % 4];
        // Here test HIGH Chall vs LOW Ref
    ...
    console.log('\n  ───────────────────────────────────────────────────────────────────────');
    console.log('  SCALING SUMMARY (HIGH Chall vs LOW Ref, fair, 8 each)');
    ...
    // Scaling law: Δ should be positive if high iters helps; we expect small at d4 (ceiling)
    const avgStrength = pairResults.reduce((a,r)=>a+r.avg,0)/pairResults.length;
    console.log(`\n  Avg strength per step (150→750, 4 steps): ${avgStrength>=0?'+':''}${avgStrength.toFixed(3)}/game`);
    console.log('  Expected ≈0 at d4 (ceiling), positive at d8');
    console.log('  ═══════════════════════════════════════════════════════════════════════\n');

    // No hard fail — just log, but ensure no huge negative bias
    expect(pairResults.every(r=>Math.abs(r.avg) < 1.5)).toBe(true);
  }, 600000);
  ...
  it('mirrors at each tier 150,300,450,600,750 — 8 each, should be ~0', async () => {
    ...
    console.log('\n  ───────────────────────────────────────────────────────────────────────');
    console.log('  MIRRORS — each tier vs itself, 8 games, fair');
    for (const lvl of levels) {
      ...
      const cW = games.filter(g=>g.winner==='C').length;
      const rW = games.filter(g=>g.winner==='R').length;
      console.log(`  d4@${String(lvl).padStart(3)} vs d4@${String(lvl).padStart(3)}  ${totalC}-${totalR} Δ=${delta>=0?'+':''}${delta} avg=${avg>=0?'+':''}${avg.toFixed(3)} W/L/D ${cW}/${rW}/${8-cW-rW}`);
