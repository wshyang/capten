...
async function generateSelfPlayParallel64(
  totalCount: number,
  baseSeed: number,
  mctsIters: number,
  mctsDepth: number,
  ...
  if (numWorkers <= 2 || totalCount <= 40) {
    return generateSelfPlayTrainingData(totalCount, baseSeed, mctsIters, mctsDepth, 64);
  }
...
export interface GoalSeeker64vs32Config {
...
export interface GoalSeeker64vs32Manifest {
...
export class GoalSeeker64vs32Manager {
  readonly config: GoalSeeker64vs32Config;
  readonly backend: FileStorageBackend;

  constructor(config: GoalSeeker64vs32Config) {
    this.config = config;
  ...
  private saveManifest(manifest: GoalSeeker64vs32Manifest): void {
    const raw = JSON.stringify(manifest, null, 2);
    const filePath = path.join(this.config.checkpointDir, 'goal_seeker_64_vs_32_manifest.json');
    const tmpPath = `${filePath}.tmp`;
    fs.writeFileSync(tmpPath, raw, 'utf8');
    fs.renameSync(tmpPath, filePath);
  }

  private loadManifest(): GoalSeeker64vs32Manifest | null {
    const filePath = path.join(this.config.checkpointDir, 'goal_seeker_64_vs_32_manifest.json');
    if (!fs.existsSync(filePath)) return null;
    try {
      return JSON.parse(fs.readFileSync(filePath, 'utf8')) as GoalSeeker64vs32Manifest;
    } catch {
      return null;
    }
  }
  ...
  private saveReplayBuffer(samples: TrainingSample[]): void {
    const filePath = path.join(this.config.checkpointDir, 'goal_seeker_64_vs_32_replay.json');
    const tmpPath = `${filePath}.tmp`;
  ...
  async run(): Promise<GoalSeeker64vs32Manifest> {
    ...
    console.log(`\n  ═══════════════════════════════════════════════════════════════════════════════════════`);
    console.log(`  CAPTEN AUTONOMOUS GOAL-SEEKING 64-CHANNEL vs. 32-CHANNEL TRAINING FLYWHEEL`);
    ...
    console.log(`  • Target Objective:  Win rate >= ${this.config.targetWinRate}% vs 32-Channel Champion across ${this.config.evalMatches} matches (${statNote})`);
    console.log(`  • Teacher Config:    64-channel MCTS d${this.config.mctsDepth}@${this.config.mctsIters} (${this.config.samplesPerGen} samples/gen, ${this.config.epochsPerGen} epochs/gen)`);
    console.log(`  ═══════════════════════════════════════════════════════════════════════════════════════`);

    let manifest = this.loadManifest();
    let replayBuffer = this.loadReplayBuffer();

    // Load benchmark Supreme Champion model (32-channel, 386,305 parameters)
    ...
    if (!supremeCkpt) {
      throw new Error(`Failed to load benchmark 32-channel champion from supreme_champion.json`);
    }
    const championModel = createCNNModel([11, 11, 32]);
    importModelCheckpoint(championModel, supremeCkpt);

    let candidateModel: tf.LayersModel;

    if (manifest && manifest.reigningModelKey) {
      console.log(`  [RESUMABILITY CHECK: Found saved GoalSeeker 64 vs 32 Manifest]`);
      console.log(`    • Status:             ${manifest.status}`);
      console.log(`    • Current Generation: Gen ${manifest.currentGeneration}`);
      console.log(`    • Replay Buffer:      ${replayBuffer.length} accumulated 64-channel training samples`);
      const ckpt = loadCheckpointFromStorage(manifest.reigningModelKey, this.backend);
      if (ckpt) {
        candidateModel = createCNNModelFromCheckpoint(ckpt);
      } else {
        const ckpt64 = promoteCheckpointTo64Channels(supremeCkpt);
        candidateModel = createCNNModelFromCheckpoint(ckpt64);
      }
    } else {
      manifest = {
        status: 'TRAINING',
        ...
        reigningModelKey: 'goal_seeker_64_vs_32_current',
        history: [],
        timestamp: Date.now(),
      };
      this.saveManifest(manifest);

      console.log(`    • Promoting 32-channel champion weights to 64-channel candidate model (788,801 parameters)...`);
      const ckpt64 = promoteCheckpointTo64Channels(supremeCkpt);
      candidateModel = createCNNModelFromCheckpoint(ckpt64);
    }
    ...
    while (manifest.status !== 'GOAL_ACHIEVED' && manifest.currentGeneration < this.config.maxGenerations) {
      ...
      console.log(`\n  ─── GOAL-SEEKER 64 vs 32 GENERATION ${gen} (Current Step: ${step}) ───`);

      if (step === 'GENERATE_DATA') {
        console.log(`    • [Step 1/3: GENERATE_DATA] Generating ${this.config.samplesPerGen} new 64-channel self-play training samples using MCTS d${this.config.mctsDepth}@${this.config.mctsIters}...`);
        const newSamples = await generateSelfPlayParallel64(
      ...
      if (manifest.currentStep === 'TRAIN_MODEL') {
        console.log(`    • [Step 2/3: TRAIN_MODEL] Supervised training on ${replayBuffer.length} 64-channel replay samples for ${this.config.epochsPerGen} epochs...`);
        ...
        saveCheckpointToStorage('goal_seeker_64_vs_32_current', ckpt, this.backend);

        manifest.currentStep = 'AUDIT_MATCHES';
        this.saveManifest(manifest);
        console.log(`      ➔ Supervised training complete & checkpoint saved to goal_seeker_64_vs_32_current.json. Advanced to step: AUDIT_MATCHES.`);
      } else {
        console.log(`    • [Step 2/3: TRAIN_MODEL] Already completed for Gen ${gen}. Loaded goal_seeker_64_vs_32_current checkpoint.`);
      }

      console.log(`    • [Step 3/3: AUDIT_MATCHES] Executing parallel audit vs. 32-Channel Supreme Champion across ${this.config.evalMatches} symmetric matches (${numWorkers} CPU threads)...`);
      ...
      console.log(
        `      ➔ 64-Channel Candidate vs. 32-Channel Champion Audit: ${evalRes.winRate.toFixed(1)}% (W/L/D: ${evalRes.w}/${evalRes.l}/${evalRes.d})`
      );
      ...
      if (evalRes.winRate >= this.config.targetWinRate) {
        ...
        console.log(
          `  New 64-Channel ResNet CNN model weight has verifiably defeated the 32-Channel Champion at ${evalRes.winRate.toFixed(1)}% (>= ${this.config.targetWinRate}%) across ${this.config.evalMatches} matches!`
        );
        manifest.status = 'GOAL_ACHIEVED';
        saveCheckpointToStorage('goal_seeker_64_vs_32_winner', ckpt, this.backend);
        this.saveManifest(manifest);
        break;
      }
...
if (process.argv[1]?.endsWith('goalSeeker_64_vs_32.ts') || process.argv[1]?.endsWith('goalSeeker_64_vs_32.js')) {
  const config: GoalSeeker64vs32Config = {
    checkpointDir: process.env.CHECKPOINT_DIR || './checkpoints/goal_seeker_64_vs_32',
    championPath: process.env.CHAMPION_PATH || './checkpoints/supreme_champion.json',
    targetWinRate: parseFloat(process.env.TARGET_WIN_RATE || '75'),
    ...
    epochsPerGen: parseInt(process.env.EPOCHS_PER_GEN || '1', 10),
    maxGenerations: parseInt(process.env.MAX_GENERATIONS || '10', 10),
    replayWindowSize: parseInt(process.env.REPLAY_WINDOW_SIZE || '120', 10),
  };

  const manager = new GoalSeeker64vs32Manager(config);
  manager
    .run()
    .then(() => {
      console.log('[goalSeeker_64_vs_32] Completed successfully.');
      process.exit(0);
    })
    .catch(err => {
      console.error('[goalSeeker_64_vs_32] Fatal error:', err);
      process.exit(1);
    });
}
