    ...
    switch (card.effect) {
      ...
      case 'clamp':
        if (enemyPieces.length > 0) {
          const highestEnergyEnemy = [...enemyPieces]
      ...
      case 'deep_breath': {
        const restingPiece = [...ourPieces]
          .filter(p => !p.isCaptain && !p.movedLastTurn)
          .sort((a, b) => b.restStreak - a.restStreak)[0] || ourPieces.find(p => !p.movedLastTurn);
        if (restingPiece) {
      ...
      case 'second_wind': {
        const lowEnergyPiece = [...ourPieces]
          .filter(p => !p.isCaptain)
          .sort((a, b) => {
            if (Math.abs(a.energy - b.energy) > 0.01) return a.energy - b.energy;
            return Math.abs(a.cell.col - 5) - Math.abs(b.cell.col - 5);
          })[0] || ourPieces[0];
      ...
      case 'reset': {
        const exhaustedPiece = [...ourPieces]
          .filter(p => !p.isCaptain)
          .sort((a, b) => {
            if (Math.abs(a.energy - b.energy) > 0.01) return a.energy - b.energy;
            return Math.abs(a.cell.col - 5) - Math.abs(b.cell.col - 5);
      ...
      case 'bait':
        if (enemyPieces.length > 0) {
          ...
          const colOffset = blocker.cell.col === 5 ? (blocker.side === 'AI' ? 1 : -1) : (blocker.cell.col < 5 ? 1 : -1);
          targetCell = { col: Math.max(0, Math.min(10, blocker.cell.col + colOffset)), row: blocker.cell.row };
          delta = 34.0;
        }
        break;
