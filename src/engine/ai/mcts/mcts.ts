...
import { getPocketScoreWithDistance } from './sideHelpers';
import { generateArchetypeClusters } from './ismcts';
...
import type { AIPlannedTurnResult } from '../interface';
export type { AIPlannedTurnResult };
...
export function applyActionToSimState(
  baseState: GameState,
  action: MCTSCandidateAction,
  rng: SeededRNG,
  actingSide: Side = 'AI',
  ...
  // Pillar II: Macro-Action Stage 0 Card Combo
  if (action.stage0Card?.card) {
    const card = action.stage0Card.card;
    if (simState.momentum[actingSide] >= card.momentumCost) {
      simState.momentum[actingSide] -= card.momentumCost;
      if (card.id === 'overclock') {
        const targetP = (action.stage0Card.targetPieceId ? simState.pieces.find(p => p.id === action.stage0Card!.targetPieceId) : null) ||
          simState.pieces.find(p => p.side === actingSide && p.hasBall);
        if (targetP) targetP.buffs.push({ id: 'buff_oc', type: 'OVERCLOCK', durationTurns: 1 });
      } else if (card.id === 'surge') {
        simState.pieces.forEach(p => { if (p.side === actingSide) p.energy = Math.min(10, p.energy + 2.0); });
      } else if (card.id === 'set_the_play') {
        if (!simState.temporaryState) simState.temporaryState = {};
        simState.temporaryState.setThePlayActive = { PLAYER: false, AI: false, ...(simState.temporaryState.setThePlayActive || {}), [actingSide]: true };
      }
    }
  }
  ...
  // Pillar II: Macro-Action Stage 2 Card Combo
  if (action.stage2Card?.card) {
    const card = action.stage2Card.card;
    if (simState.momentum[actingSide] >= card.momentumCost) {
      simState.momentum[actingSide] -= card.momentumCost;
      if (card.id === 'drain') {
        const enCarrier = simState.pieces.find(p => p.side === enemySide && p.hasBall);
        if (enCarrier) enCarrier.energy = Math.max(0, enCarrier.energy - 2.0);
      } else if (card.id === 'clamp') {
        const enTop = simState.pieces.filter(p => p.side === enemySide && !p.isCaptain).sort((a, b) => b.energy - a.energy)[0];
        if (enTop) enTop.buffs.push({ id: 'buff_clamp', type: 'CLAMP', durationTurns: 1 });
      } else if (card.id === 'threaded_pass') {
        if (!simState.temporaryState) simState.temporaryState = {};
        simState.temporaryState.threadedPassActive = { PLAYER: false, AI: false, ...(simState.temporaryState.threadedPassActive || {}), [actingSide]: true };
      } else if (card.id === 'steady_hands') {
        if (!simState.temporaryState) simState.temporaryState = {};
        simState.temporaryState.steadyHandsActive = { PLAYER: false, AI: false, ...(simState.temporaryState.steadyHandsActive || {}), [actingSide]: true };
      }
    }
  }
...
function computePolicyPrior(action: MCTSCandidateAction, state: GameState, _actingSide: Side): number {
  if (action.stage0Card?.card || action.stage2Card?.card) {
    return 0.55;
  }
  if (action.throwTargetPieceId) {
    const targetPiece = state.pieces.find(p => p.id === action.throwTargetPieceId);
    if (targetPiece?.isCaptain) return 0.45;
    if (action.moves.length >= 2) return 0.38;
    if (action.moves.length === 1) return 0.32;
    if (targetPiece?.isBlocker) return 0.10;
    return 0.25;
  }
  if (action.moves.length >= 2) return 0.28; // Joint defensive formation
  if (action.moves.length === 1) return 0.22; // Single defensive move
  return 0.10; // Anchor / Hold
}

function enrichActionsWithPriors(actions: MCTSCandidateAction[], state: GameState, actingSide: Side): MCTSCandidateAction[] {
  return actions
    .map(a => ({
      ...a,
      prior: a.prior ?? computePolicyPrior(a, state, actingSide),
    }))
...
export function runSeededMCTS(state: GameState, rng: SeededRNG, actingSide: Side = 'AI'): AIPlannedTurnResult {
  ...
  const rawCandidates = generateJointCandidateActions(state, posture, actingSide);
  const allCandidates = enrichActionsWithPriors(rawCandidates, state, actingSide);
  ...
  if (
    lastTreeCache &&
    lastTreeCache.turn === state.turn - 1 &&
    lastTreeCache.actingSide === actingSide &&
    lastTreeCache.seed === state.seed &&
    state.turn > 1
  ) {
    for (const ch of lastTreeCache.root.children) {
      if (ch.children.length > 0) {
        rootNode.children = ch.children;
        break;
      }
    }
  }
  ...
  const archetypeClusters = generateArchetypeClusters(state, rng, actingSide, Math.max(3, config.ismctsSamples || 10));
  const hiddenSide: Side = actingSide === 'AI' ? 'PLAYER' : 'AI';

  for (let i = 0; i < iterations; i++) {
    state.hands[hiddenSide] = archetypeClusters[i % archetypeClusters.length];
  ...
  const bestActionDescription = formatActionDescription(bestAction, throwAction);
...
function selectBestChild(node: MCTSNode, c: number): MCTSNode {
  ...
  const parentVisitsSqrt = Math.sqrt(node.visits + 1);

  for (const child of node.children) {
    const q = child.totalScore / (child.visits || 1);
    const u = c * child.prior * (parentVisitsSqrt / (1 + child.visits)) * 40.0;
    const puct = q + u;
...
export function generateJointCandidateActions(
  ...
  for (const p of mobilePieces) {
    ...
    if (carrier) {
      ...
      reachable.sort((a, b) => {
        ...
        return getPocketScoreWithDistance(b.cell, actingSide, distB) - getPocketScoreWithDistance(a.cell, actingSide, distA);
      });
      ...
      reachable.sort((a, b) => {
        ...
        const distA = Math.hypot(a.cell.col - enemyScoringCell.col, a.cell.row - enemyScoringCell.row);
        const distB = Math.hypot(b.cell.col - enemyScoringCell.col, b.cell.row - enemyScoringCell.row);
        return distA - distB;
      });
    }
  ...
  if (carrier) {
    ...
    if (captain && !captain.hasBall && !state.isRestartPhase?.[actingSide]) {
      ...
      if (otherRunners.length >= 2) {
        ...
        if (reach1.length > 0 && reach2.length > 0 && !areCellsEqual(reach1[0].cell, reach2[0].cell)) {
          candidates.push({
            moves: [
  ...
  // Pairwise Joint Movements across mobile piece pairs (wings, center-line, base)
  for (let i = 0; i < mobilePieces.length; i++) {
    for (let j = i + 1; j < Math.min(i + 3, mobilePieces.length); j++) {
      const p1 = mobilePieces[i];
      const p2 = mobilePieces[j];
      ...
      if (reach1.length > 0 && reach2.length > 0) {
        ...
        if (!areCellsEqual(m1.cell, m2.cell)) {
          candidates.push({
            ...
            description: `Joint Defense Formation: ${p1.id} & ${p2.id}`,
          });
        }
      }
    }
  ...
  // Pillar II: Macro-Action Card Combos ([Card + Move + Throw])
  if (state.momentum[actingSide] >= 1) {
    const stage0 = evaluateStage0Cards(state, actingSide);
    const stage2 = evaluateStage2Cards(state, actingSide);
    const macroCandidates: MCTSCandidateAction[] = [];
    const topCarrierActions = candidates.filter(c => !!c.throwTargetPieceId).slice(0, 3);
    for (const base of topCarrierActions) {
      if (stage0.card) {
        macroCandidates.push({
          ...base,
          stage0Card: stage0,
          description: `[${stage0.card.name}] ${base.description || ''}`,
        });
      }
      if (stage2.card && stage2.evaluatedDelta >= 30) {
        macroCandidates.push({
          ...base,
          stage2Card: stage2,
          description: `[${stage2.card.name}] ${base.description || ''}`,
        });
      }
    }
    candidates.push(...macroCandidates);
  }
