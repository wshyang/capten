...
/**
 * Calculates positional pocket score with Euclidean distance penalty for offensive drive ranking.
 */
export function getPocketScoreWithDistance(cell: Cell, actingSide: Side, euclideanDist: number): number {
  const scoring = getOurScoringCell(actingSide);
  const distToScoring = Math.abs(cell.row - scoring.row);
  if (distToScoring === 2 || distToScoring === 3) return 50.0 - euclideanDist * 1.5;
  if (distToScoring === 1 || distToScoring === 4) return 38.0 - euclideanDist * 1.5;
  if (distToScoring === 0) return 28.0 - euclideanDist * 1.5;
  if (distToScoring === 5) return 15.0 - euclideanDist;
